...
<div class="mx-auto max-w-4xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-slate-900">Edit Paket</h2>
        <p class="mt-1 text-sm text-slate-500">Perbarui paket harga untuk {{ $product->name }}.</p>
    </div>

    <form action="{{ route('admin.products.packages.update', [$product, $package]) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid gap-6 md:grid-cols-2">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $package->name) }}" class="w-full rounded-xl border border-slate-300 px-3 py-2" required>
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">Harga (Rp)</label>
                <input type="number" name="price" value="{{ old('price', $package->price) }}" class="w-full rounded-xl border border-slate-300 px-3 py-2" min="0" required>
            </div>
        </div>
        ...
        <div class="grid gap-6 md:grid-cols-2">
            <div>
                ...
                <select name="duration_days" class="w-full rounded-xl border border-slate-300 px-3 py-2">
                    <option value="30" {{ old('duration_days', $package->duration_days) == '30' ? 'selected' : '' }}>30 hari</option>
                    <option value="90" {{ old('duration_days', $package->duration_days) == '90' ? 'selected' : '' }}>90 hari</option>
                    <option value="180" {{ old('duration_days', $package->duration_days) == '180' ? 'selected' : '' }}>180 hari</option>
                    <option value="365" {{ old('duration_days', $package->duration_days) == '365' ? 'selected' : '' }}>365 hari</option>
                    <option value="3650" {{ old('duration_days', $package->duration_days) == '3650' ? 'selected' : '' }}>Seumur Hidup</option>
                </select>
            </div>
            <div>
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full rounded-xl border border-slate-300 px-3 py-2">{{ old('description', $package->description) }}</textarea>
        </div>

        <div>
            <label class="mb-1 block text-sm font-medium text-slate-700">Fitur yang termasuk</label>
            <textarea name="features" rows="5" class="w-full rounded-xl border border-slate-300 px-3 py-2">{{ old('features', implode($package->features ?? [], PHP_EOL)) }}</textarea>
